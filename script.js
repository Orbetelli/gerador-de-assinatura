...
async function processImage() {
    const removeFilters = document.getElementById('removeFilters').checked;
    ...
    return new Promise((resolve) => {
        ...
        if (removeFilters) {
            // Modo sem filtros - apenas remove fundo muito claro
            for (let i = 0; i < data.length; i += 4) {
                ...
                // Remove apenas pixels muito claros (fundo branco)
                if (brightness > 240) {
                    data[i + 3] = 0;
                }
                // Mantém cores originais para o resto
            }
        } else {
            // Modo com filtros - aplica preto e branco
            for (let i = 0; i < data.length; i += 4) {
                const r = data[i];
                const g = data[i + 1];
                const b = data[i + 2];
